...
test.describe('The Fork - Full User Journey', () => {
  test('should display fork setup screen on initial load', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="app-title"]')).toContainText('The Fork');
    await expect(page.locator('[data-testid="app-tagline"]')).toContainText('One decision');
  ...
  test('should enable start button when fork statement is entered', async ({ page }) => {
    await page.goto('/');
  ...
  test('should transition to chat view after starting conversation', async ({ page }) => {
    await page.goto('/');
    
    // Fill fork statement
    await page.locator('[data-testid="fork-statement-input"]').fill('I chose law instead of medicine.');
    ...
    await expect(page.locator('[data-testid="fork-summary-bar"]')).toBeVisible();
    
    // Verify chat window is visible
    await expect(page.locator('[data-testid="chat-window"]')).toBeVisible();
    
    // Verify empty state message
    await expect(page.locator('[data-testid="chat-empty-state"]')).toContainText('interrogation room is ready');
  });
  ...
  test('should allow message composition and sending', async ({ page }) => {
    ...
    await page.locator('[data-testid="fork-statement-input"]').fill('I chose to move to the city instead of staying in my hometown.');
    await page.locator('[data-testid="open-other-door-button"]').click();
    ...
    await page.locator('[data-testid="message-list"]').waitFor({ state: 'attached' });
  });
  ...
  test('should handle Enter key to send message', async ({ page }) => {
    ...
    // Setup conversation
    await page.locator('[data-testid="fork-statement-input"]').fill('I chose to have children instead of pursuing my career.');
    await page.locator('[data-testid="open-other-door-button"]').click();
  ...
  test('should handle Shift+Enter for newline', async ({ page }) => {
    ...
    // Setup conversation
    await page.locator('[data-testid="fork-statement-input"]').fill('I chose academia instead of industry.');
    await page.locator('[data-testid="open-other-door-button"]').click();
  ...
  test('should open reset modal when reset button is clicked', async ({ page }) => {
    await page.goto('/');
    
    // Start conversation
    await page.locator('[data-testid="fork-statement-input"]').fill('I chose to travel instead of settling down.');
    await page.locator('[data-testid="open-other-door-button"]').click();
    ...
    await expect(page.locator('[data-testid="reset-modal-title"]')).toContainText('Burn it');
  });
  ...
  test('should reset timeline when confirmed', async ({ page }) => {
    ...
    await page.locator('[data-testid="fork-statement-input"]').fill(forkStatement);
    await page.locator('[data-testid="open-other-door-button"]').click();
    ...
    await expect(page.locator('[data-testid="fork-statement-input"]')).toHaveValue('');
  });
