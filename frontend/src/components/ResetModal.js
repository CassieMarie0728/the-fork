...
export const ResetModal = ({ open, onCancel, onConfirm }) => {
  ...
  useEffect(() => {
    ...
    if (open) {
      ...
      // Focus the cancel button (the "safe" option) when the modal opens
      setTimeout(() => {
      ...
      <div className="w-full max-w-lg rounded-3xl border border-white/10 bg-ink/90 p-6 shadow-2xl shadow-black/60">
        ...
        <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          <button
            ...
            type="button"
            onClick={onCancel}
            className="rounded-2xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-zinc-100 transition-colors duration-200 hover:bg-white/10 focus-visible:ring-2 focus-visible:ring-white/60 focus-visible:outline-none"
          >
            Keep This Timeline
          </button>
          <button
            ...
            type="button"
            onClick={onConfirm}
            className="rounded-2xl bg-crimson px-4 py-2 text-sm font-semibold text-white transition-colors duration-200 hover:bg-crimson/90 focus-visible:ring-2 focus-visible:ring-white/60 focus-visible:outline-none"
          >
            Burn This Timeline
          </button>
        </div>
